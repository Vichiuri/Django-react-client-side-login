...
class DistriButorUserSerializer(serializers.ModelSerializer):
    ...
    def get_dist_options(self, obj):
        dist_settings = DistributorOptions.objects.filter(distributor=obj.distributor)
...
class RetailerStaticsViewSerializer(serializers.ModelSerializer):
    ...
    def get_order_count(self, obj):
        retail_orders = RetailOrders.objects.filter(retailer=obj, when_approved__isnull=False)
...
class RetailerDistributorSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Distributor
        fields = ['id', 'name', 'kra', 'email','logo', 'dist_options']

class RetailerUserProfileSerializer(serializers.ModelSerializer):
...
class RetailerUserSerializer(serializers.ModelSerializer):
    ...
    def get_retail_profile(self, obj):
        v_retailer = RetailerProfile.objects.filter(retailer=obj.retailer)

        if v_retailer.first():
            if obj.default_distributor:
                ret_profile = v_retailer.filter(distributor = obj.default_distributor)
                v_retailer = ret_profile
                
            return RetailerUserProfileSerializer(v_retailer.first()).data
        else:
            return None
...
class VariationOptionImagesSerializer(serializers.ModelSerializer):
    class Meta:
        model = VariationOptionImages
        fields = '__all__'

class VariationOptionVideosSerializer(serializers.ModelSerializer):
    class Meta:
        model = VariationOptionVideos
        fields = '__all__'
...
class VariationOptionsSerializer(serializers.ModelSerializer):
    variation = VariationSerializer(many=False, read_only=True)
...
class VariationSerializer(serializers.ModelSerializer): 
    attributes = VariationOptionsSerializer(many=True, read_only=True) 
    class Meta:
        model = Variation
        fields = '__all__'
...
class VariationVideoSerializer(serializers.ModelSerializer):
    class Meta:
        model = VariationVideo
        fields = '__all__'
class RetailerProductsSerializer(serializers.ModelSerializer):
    variation_image = VariationImageSerializer(many=True, read_only=True)
    variation_video = VariationVideoSerializer(many=True, read_only=True)
    product_images = ProductImageSerializer(many=True, read_only=True)
    category = ProductCategorySerializer(many=False, read_only=True)
    units = MUnitSerializer(many=False, read_only=True)
    colors = CustomColorsSerializer(many=True, read_only=True)
    ...
    test_field= serializers.SerializerMethodField(required=False, allow_null=True,default=True)

    variations=VariationOptionsSerializer(many=True, allow_null=True,read_only=True)

    def get_is_favorite(self, obj):
        retail_user = self.context.get("retail_user")
        if retail_user and obj in retail_user.favorite_products.all():
            return True
        return False
    ...
    def get_cart_qty(self, obj):
        ...
        retail_user = self.context.get("retail_user")
        if retail_user:
            cart = RetailerCart.objects.filter(retailer=retail_user.retailer).first()
            if cart is None:
                return cart_qty
...
class PriceProductSerializer(serializers.ModelSerializer):
    class Meta:
        model = Product
        fields = ['id', 'name', 'units', 'price', 'stock_qty', 'colors', 'brand', 'size']
...
class PriceOfferDetailSerializer(serializers.ModelSerializer):
    x_item = ProductSerializer(many=False, read_only=True)
    y_item = ProductSerializer(many=False, read_only=True)
...
class RetOrderProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = RetailerProfile
        fields = ['id', 'contact_name', 'contact_phone', 'contact_email', 'city', 'salesman', 'retailer', 'price_level']
...
class RetailOrdersSerializer(serializers.ModelSerializer):
    ...
    def get_salesman(self, obj):
        ...
        if salesman is None:
            salesman = SalesMan.objects.filter(distributor=obj.distributor).first()
        return {
            'id': salesman.id,
    ...
    def get_retailer(self, obj):
        retail_profile = RetailerProfile.objects.filter(distributor=obj.distributor, retailer=obj.retailer).first()
...
class RetailerViewOrdersSerializer(serializers.ModelSerializer):
    ...
    def get_distributor(self, obj):
        profile = RetailerDistributorSerializer(obj.distributor, many=False).data
        return profile
...
class RetailOrdersStatsSeriliazer(serializers.ModelSerializer):
    ...
    class Meta:
        model = RetailOrders
        fields = ['id', 'ret_orders', 'total_cost', 'when_placed', 'when_approved', 'when_declined', 'when_dispatched']
...
class MobileOrderSerializer(serializers.ModelSerializer):
    ...
    variations = VariationOptionsSerializer(many=False, allow_null=True)
    ...
    #     # return VariationOptionsSerializer(obj.variations).data
    #     return str(obj.order_price)
