...
class SalesManApi(generics.RetrieveAPIView):
    ...
    def get(self, request):
        ...
        if query:
            sales_people = SalesMan.objects.filter(distributor=distributor).order_by('-id')
            ...
            querySet = SalesMan.objects.filter(distributor=distributor).order_by('-id')
...
class SalesTargetApi(generics.RetrieveAPIView):
    ...
    def get(self, request):
        ...
        query = self.request.query_params.get("query")
        rows = self.request.query_params.get("rows")
        querySet = SalesTargetGroup.objects.filter(distributor=distributor).order_by('-id')
    ...
    def put(self, request):
        json_data = json.loads(request.body)
        sale_target = SalesTargetGroup.objects.get(id=json_data['id'])

        sale_target.name = json_data['name']
        sale_target.period = json_data['period']
        sale_target.target_sales = json_data['target_sales']
        sale_target.reward_money = json_data['reward_money']
        sale_target.reward_per = json_data['reward_per']

        if json_data['sale_people']:
            sale_people = json.loads(json_data['sale_people'])
