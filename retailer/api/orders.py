...
class RetailerDistOrderApi(generics.RetrieveAPIView):
    ...
    def post(self, request):
        ...
        order = RetailOrders(distributor=distributor, retailer=retailer, total_cost=json_data['total_cost'],
                             salesman=salesman, payment_terms=json_data['payment_terms'], price_level=priceLevel,
...
class UpdateRetailOrderApi(generics.RetrieveAPIView):
    ...
    def put(self, request):
        ...
        if json_data['level']:
            ...
            retailer_profile = RetailerProfile.objects.filter(retailer=order.retailer, distributor=distributor).first()
            level = retailer_profile.price_level
        qty = json_data['qty']
...
class RetailerSearchOrderApi(generics.RetrieveAPIView):
    ...
    def get(self, request):
        ...
        orders = RetailOrders.objects.filter(distributor=distributor).order_by('-id')
...
class DistOrderApi(generics.RetrieveAPIView):
    ...
    def get(self, request):
        ...
        if self.request.query_params.get("retailer_id"):
            ...
            querySet = DistOrder.objects.filter(distributor=distributor, retailer=retailer, placed=False)
        else:
    ...
    def put(self, request):
        ...
        for offer in retail_order.offers.all():
            if offer.scheme == "BnXYF":
                current_order = dist_orders.filter(product=offer.y_item, order_price=0)
