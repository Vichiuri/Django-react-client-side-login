from distributor.models.distributor import DistributorOptions
...
class CustomColorsApi(generics.RetrieveAPIView):
    ...
    def get(self, request):
        ...
        if query:
            colors = CustomColors.objects.filter(Q(label__icontains=query))[:25]
        else:
...
class WorldCitiesApi(generics.RetrieveAPIView):
    ...
    def get(self, request):
        ...
        if country_id:
            ...
            cities = WorldCity.objects.filter(Q(country_name__icontains=query) | Q(city_name__icontains=query))
        else:
...
class BusinessSettingsApi(generics.RetrieveAPIView):
    ...
    def get(self, request):
        ...
        businessSettings = BusinessSettings.objects.filter(distributor=distributor)
