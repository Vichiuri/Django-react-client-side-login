...
class LoginApi(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if account:
            ...
            # * This is to check if the subcription is current over
            # * Do not migrate without consoltation. 
            ...
            # ? Redirect for the netbot authentication
            return Response({
                "message": "Redirecting",
    ...
    * View for forgot password for netbot auth and distributors
    ...
    def put(self, request):
        ...
        if querySet.first():
            ...
            timeStamp = float(token[25:])
            expiration_date =  datetime.fromtimestamp(timeStamp)
            # * Check expiration to make sure token is 
            if expiration_date.replace(tzinfo=None) > datetime.now().replace(tzinfo=None):
