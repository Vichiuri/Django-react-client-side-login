...
export const retailerDashBoardItems =
  (distributor_id) => (dispatch, getState) => {
    ...
    Axios.get(
      `/api/retailer_dashboard_view/?distributor_id=${distributor_id}`,
      config
    )
      .then((res) => {
        dispatch({
...
export const fetchCategoryProducts = (category_id) => (dispatch, getState) => {
  ...
  Axios.get("/api/retailer_category_products/", config)
    .then((res) => {})
    .catch((error) => {
      const errors = {
        responseMessage: error.response.data,
...
export const retailerNewArrivalProducts =
  (distributor_id) => (dispatch, getState) => {
    ...
    Axios.get(
      `/api/retailer_new_arrivals/?distributor_id=${distributor_id}`,
      config
    )
      .then((res) => {
        dispatch({
