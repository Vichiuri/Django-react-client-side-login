    ...
    def post(self, request):
        try:
            ...
            if action == 'confirm':
                ...
                ret_order = RetailOrders.objects.filter(id=ret_order_id).first()
    ...
    def get(self, request):
        try:
            ...
            if salesman is not None:
                ...
                orders = orders.filter(salesman=salesman, distributor=distributor)
            ...
            if 'from' in request.GET.keys():
                ...
                if from_:
                    try:
                        print("\nDate We Go", from_, "\n")
                        time_s = datetime.datetime.fromtimestamp(int(from_) / 1000.0)
            ...
            if 'to' in request.GET.keys():
                ...
                if to:
                    try:
                        time_s = datetime.datetime.fromtimestamp(int(to) / 1000.0)
            ...
            if action == 'cart_qty':
                ...
                if salesman:
                    ...
                    for ret in cart.retailers:
                        ...
                        data_r['cart'] = SalesCartSerializer(cart, retailer_id=ret.id).data
    ...
    def put(self, request):
        try:
            ...
            for retail in retailers:
                orders = cart.orders.filter(retailer=retail, placed=False).all()
                if orders.count() < 1:
                    return Response({"message": "No  Products In Cart"}, status=404)
                for distributor in distributors:
                    orders = cart.orders.filter(retailer=retail, placed=False, distributor=distributor)
                    ...
                    for order in orders.all():
                        ...
                        print("Order Delete After use on cart", cart.orders.remove(order), order.delete())
            ...
            print("Clearing Orders", cart.orders.filter(retailer__in=retailers).delete())
