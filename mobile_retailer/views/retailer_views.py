...
class CartApiView(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        product_price = commons.retailer_price_list_product_price(product.distributor, retailer, product, qty)
    ...
    def put(self, request):
        ...
        for dist in distributors:
            ...
            if mobile_orders.count() >= 1:
                ...
                order_no = 1
                prev_orders = RetailOrders.objects.filter(distributor=dist).order_by('-id')
    ...
    def delete(self, request):
        ...
        if salesman is not None:
            mobile_order = MobileOrder.objects.filter(id=order_id, salesman=salesman)
            ...
            mobile_order = MobileOrder.objects.filter(id=order_id, retailer=request.user.user_retailer.retailer).first()
            if mobile_order is None:
                return Response({"message": " You are UnAuthorised To delete the Order"}, status=404)
            if mobile_order.offer_id:
                if mobile_order.offer_id > 0:
...
class ProductsByCategory(generics.GenericAPIView):
    ...
    def get(self, request, category_id):
        ...
        products = Product.objects.filter(category_id=category_id, active=True).order_by('name')
        ...
        if user.salesman:
            ...
            prs = ProductSerializerM(c_page.pageList, many=True, retailer_id=retailer_id).data
        return Response({
            "products": prs,
...
class PurchaseOnOffer(generics.GenericAPIView):
    ...
    def post(self, request):
        ...
        mobile_order = MobileOrder()
        x_item = offer.x_item
        mobile_order_check = MobileOrder.objects.filter(product=x_item, applied_offer__isnull=False).first()

        if mobile_order_check is not None:
            mobile_order = mobile_order_check
            mobile_order.retailer = retailer

        product_price = commons.retailer_price_list_product_price(x_item.distributor, retailer, x_item, qty)
        x_qty = offer.x_amt
...
class MobileNotificationsView(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        today = timezone.now()
        notifications = user.user_notifications.filter(show_from__lte=today, show_to__gte=today).order_by('-id').all()
        return Response({
    ...
    def get(self, request):
        ...
        if os.path.exists(file_path):
            with open(file_path, 'rb') as fh:
                ...
                response['Content-Disposition'] = 'inline; filename=' + "netbot_app.apk"
                return response
        raise Http404
