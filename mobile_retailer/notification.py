...
class NotifyRetailer:
    ...
    def notify_device(self):
        ...
        result = push_service.notify_multiple_devices(registration_ids=devices, message_title=message_title,
                                                      ...
                                                      data_message=PushNotificationSerializer(self.notification).data,
...
class NotifyRetailerProduct:
    ...
    def __init__(self, distributor_id, product_id):
        ...
        self.retailers = Retailer.objects.filter(distributors__in=[self.distributor]).all()
        self.retailer_users = RetailerUser.objects.filter(retailer__in=self.retailers, device_id__isnull=False).all()
...
def order_notification(order: RetailOrders, status: str):
    ...
    notif = MNotification.objects.filter(status="Order", order_id=order.id).first()
    if notif is None:
        notif = MNotification()
    notif.status = "Order"
    ...
    if salesman is not None:
        user = User.objects.filter(email=salesman.email).first() # Add salesman as user meant for the notification
        if user is not None:
            users.append(user)
    if len(users) > 0:
        if notif.id is None:
            ...
            notif.users.add(*users)
            notif.save()
    print(NotifyRetailer(distributor_id=notif.distributor.id, push_notification_id=notif.id).notify_device()) # Fire the notification
