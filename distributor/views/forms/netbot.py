...
from distributor.commons import addCustomUnits
...
class AddDistributorView(View):
    def get(self, request):
        ...
        context = {"page_title": "Add Distributor", "form": form, 'subscription': subscription}
    ...
    def post(self, request):
        ...
        try:
            ...
            if 'id' in request.POST.keys():
                ...
                dist = DistForm(request.POST, request.FILES, instance=distributor)
                user = User.objects.filter(email=str(request.POST['email'])).first()
                ...
                if dist.is_valid() and user_f.is_valid():
                    ...
                    context = {"page_title": "Add Distributor Failed", "form": dist}
                    return render(request, 'NewTheme/add-distributor.html', context=context)

            elif form.is_valid():
                user = None
                distributor = form.save(commit=False)
                retailer_ = Retailer.objects.filter(pin_no=distributor.kra, email=request.POST['email'])
    ...
    def post(self, request):
        ...
        if user_.is_valid() and net_user_.is_valid():
            ...
            if send_html_email([user.email], "Netbot Account Creation", 'emails/netbot_user_password.html',
                ...
                net_user.user = user
                net_user.save()
                messages.success(request, "User Saved Successfully,Login Credentials Send To Their Email")
                return redirect('distributor:user-list')
            user.set_password("user123")
            user.save()
            net_user = net_user_.save(commit=False)
