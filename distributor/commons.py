...
def random_password():
    ...
    result_str = result_str + str((''.join(random.choice(lettersLow) for i in range(15))))
    result_str = result_str + str((''.join(random.choice(numbers_) for i in range(9))))
...
def addCustomUnits(distributor):
    ...
    for unit in unitsJson:
        munit = MUnit(distributor=distributor, name=unit['name'], symbol=unit['symbol'])
        munit.save()
...
def checkCurrentProgress(distributor):
    ...
    """
    email_settings = DistEmailSettings.objects.filter(distributor=distributor).first()
...
def distributor_price_list(distributor, level, product, qty):
    ...
    try:
        price = product.price
        dist_settings = DistributorOptions.objects.filter(distributor=distributor).first()
        ...
        amount = (price_list.rate - price_list.discount_amount) * ((100 - price_list.discount_percent) / 100)
...
def retailer_price_list_product_price(distributor, retailer, product, qty,variant=None):
    """
    ...
    try:
        ...
        price_list = PriceList.objects.filter(distributor=distributor, product=product,variation=variations,
            ...
            '-max_amount').first()
        if price_list is None and variations is None:
            # print("Quantity", qty, "For Product", product.name, "No Match")
            return price
        if price_list is None and variations:
            return variations.selling_price
        # print("\n\n Applied Price List", str(price_list), price_list.product.name)
        amount = (price_list.rate - price_list.discount_amount) * ((100 - price_list.discount_percent) / 100)
        price = Money(str(amount), price.currency)
...
def random_price_lists(product, price_level_id):
    ...
    for i in range(random.randint(0, 15)):
        ...
        price_list.date_from = datetime(2021, random.randint(1, 6), random.randint(1, 28))
        ...
        price_list.discount_amount = float(random.randint(0, int(product.price.amount / 5)))
        price_list.save()
...
def customDecrypt(txt):
    ...
    try:
        ...
        # ? Encryption key should be added in settings file
