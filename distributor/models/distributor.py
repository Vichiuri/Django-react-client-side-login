"""
Most Models Live Here
All modles Concerning the distributor data
"""
...
def validate_phone(value):
    ...
    if len(val) < 10 or len(val) > 15:
        raise ValidationError(
            _lazy_t('%(value)s Length Must Be at least 12 and not more than 15 Characters'),
            params={'value': value},
        )
...
def validate_pin(value):
    val = str(value)
    if len(val) < 11 or len(val) > 11:
        raise ValidationError(
            _lazy_t('%(value)s Length Must Be 11 Characters'),
            params={'value': value},
        )
    if val[0].isdigit():
        raise ValidationError(
            _lazy_t('%(value)s Must Start With A letter'),
            params={'value': value},
        )
    if val[len(val) - 1].isdigit():
        raise ValidationError(
            _lazy_t('%(value)s Must End With A Letter'),
            params={'value': value},
        )
...
class PCategory(models.Model):
    ...
    category_pic = models.ImageField(null=True, blank=True, verbose_name="Category Picture")
    brief_description = models.CharField(verbose_name='Brief Description', max_length=255, null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        if self.id:
            if not self.can_be_saved():
                raise ValidationError("Could Not Save Category.... System Could Have Run Into Circular Category Trees")
...
class SubCategory(models.Model):
    ...
    category = models.ForeignKey("distributor.PCategory", verbose_name="Category", on_delete=models.CASCADE)
    name = models.CharField(verbose_name="Sub Category", max_length=50)
    sub_category_pic = models.ImageField(verbose_name="SubCategory Pic", null=True, blank=True)
...
class Retailer(models.Model):
    ...
    pin_no = models.CharField(max_length=11, unique=True, verbose_name="Retailer Tax Pin", validators=[validate_pin, ])
    name = models.CharField(verbose_name="Client Name", max_length=50)
    phone = models.CharField(max_length=15, verbose_name="Retailer Phone", validators=[validate_phone])
    email = models.EmailField(max_length=50, verbose_name="Retailer Email", unique=True)
    pic = models.ImageField(verbose_name='Retailer Image', upload_to="retailer/profile/", blank=True, null=True)
...
class Product(models.Model):
    ...
    #    h                             verbose_name="Product Sub Category")
    sku = models.CharField(verbose_name='Product Sku', max_length=100, blank=True, null=True)
    active = models.BooleanField(default=True, verbose_name='Product Active')
    weight = models.CharField(verbose_name='Product Weight', max_length=100, null=True, blank=True)
    track_stock = models.BooleanField(default=True, verbose_name='Product Stock')
    ...
    stock_qty = models.PositiveIntegerField(verbose_name="Quantity In Stock", default=0, blank=True)
    colors = models.ManyToManyField('distributor.CustomColors', verbose_name='assign colors', blank=True)
    brand = models.TextField(verbose_name="Product Brand", null=True, blank=True)
    ...
    is_new_arrival = models.BooleanField(verbose_name="Is New Arrival", default=False)
    ...
    variation_image =  models.ManyToManyField('distributor.VariationImage', verbose_name="variant image")
    variation_video =  models.ManyToManyField('distributor.VariationVideo', verbose_name="variant video")
    variations =  models.ManyToManyField(
        "distributor.VariationOptions", verbose_name="Product Variations", blank=True)
    ...
    def price_slabs(self):
        ...
        price_list = PriceList.objects.filter(product=self, date_from__lte=date_today)
        return price_list

    def slabs(self, price_l):
        price_ls = self.price_slabs.filter(price_level=price_l).order_by('min_amount').all()
        prices = list()
        for price in price_ls:
            data = dict()
            data['min'] = price.min_amount
            data['max'] = price.max_amount
            data['rate'] = str(Money(str(price.rate), price.product.price.currency))
            data['discount_amount'] = str(Money(str(price.discount_amount), price.product.price.currency))
...
#     product = models.ForeignKey("distributor.Product", verbose_name="Product Variations", on_delete=models.CASCADE)
#     name = models.CharField(verbose_name='variation name', max_length=50)
#     label = models.CharField(verbose_name='variation label', max_length=250)
#     variations = models.TextField(verbose_name='all variations', null=True, blank=True)
...
class DistOrder(models.Model):
    distributor = models.ForeignKey("distributor.Distributor", verbose_name="Distributor", on_delete=models.CASCADE)
    retailer = models.ForeignKey("distributor.Retailer", verbose_name="Retailer", on_delete=models.CASCADE)
    product = models.ForeignKey("distributor.Product", on_delete=models.PROTECT, related_name="order_product")
    ...
    discount_amount = models.PositiveIntegerField(verbose_name=" Discount Amount ", default=0)
    ...
    remaining_amount = models.PositiveIntegerField(verbose_name='remaining price', default=0)
    ...
    delivered_qty = models.PositiveIntegerField(verbose_name='delivered_qty', default=0)
...
class RetailOrders(models.Model):
    ...
    ref_number = models.CharField(
        verbose_name="Order Reference number",max_length=50, default="", editable=False)
    ...
    status = models.CharField(max_length=50,
                              choices=(("Pending", "Pending"), ("Dispatched", "Dispatched"), ("On Hold", "On Hold"),
                                       ("Declined", "Declined"), ("Approved", "Approved"), ("Partial", "Partial"),
                                       ("CANCELED", "CANCELED")),
                              default="Pending",
                              verbose_name="Order Status")
    when_placed = models.DateTimeField(verbose_name="Order Date", auto_now_add=True)
    when_approved = models.DateTimeField(verbose_name="When Approved", null=True, blank=True)
    when_declined = models.DateTimeField(verbose_name="When Declined", null=True, blank=True)
    when_dispatched = models.DateTimeField(verbose_name="When Dispatched", null=True, blank=True)
    when_cancelled = models.DateTimeField(verbose_name="When Cancelled", null=True, blank=True)
    when_held = models.DateTimeField(verbose_name="When Held", null=True, blank=True)
    note = models.TextField(verbose_name="Reasons For  Action", null=True, blank=True, default=" ")
    ...
    payment_terms = models.CharField(max_length=100, verbose_name="Payment Terms", default="Cash")
    offers = models.ManyToManyField("distributor.PriceOffer", blank=True, editable=False)
    price_level = models.ForeignKey("distributor.PriceLevel", on_delete=models.PROTECT, null=True, blank=True)
    confirmed_delivery = models.BooleanField(verbose_name="Delivery Confirmed", default=False)
    created_by = models.ForeignKey("distributor.DistUsers", verbose_name="created by", on_delete=models.CASCADE,
                                   null=True, blank=True)
    history = HistoricalRecords()

    # def save(self, *args, **kwargs):
    #     super(RetailOrders).save()
    ...
    def save(self, *args, **kwargs):
        ...
        if self.id is None:
            ...
            notif.name = f"{order.distributor.name} Order Id #{order.ref_number} Placement Success"
            notif.display_text = notif.name
            notif.detail = f"Order {str(datetime.date.today())} Placement Success Order Id #{order.ref_number} "
            ...
            super(RetailOrders, self).save(*args, **kwargs)
            notif.order_id = self.id
            notif.name = f"{order.distributor.name} Order Id {order.ref_number} Placement Success"
            notif.display_text = notif.name
            notif.detail = f" Order {str(datetime.date.today())} Placement Success Order Id #{order.ref_number} "
            notif.users.add(*users)
            notif.save()
            from mobile_retailer.notification import NotifyRetailer
            print(NotifyRetailer(distributor_id=notif.distributor.id, push_notification_id=notif.id).notify_device())
        # self.ref_number = self.set_ref_no
        super(RetailOrders, self).save(*args, **kwargs)
    ...
    #     order_no = 1
    #     prev_orders = RetailOrders.objects.filter(distributor=self.distributor).order_by('-id')
    ...
    def transport_details(self):
        transports = list()
        partial_deliveries = PartialDelivery.objects.filter(previous_order_id=self.id).all()
        ...
        for partial_delivery in partial_deliveries:
            ...
            for order in partial_delivery.current_orders.all():
                ...
                order_['pic']=None
                image=ProductImage.objects.filter(product=order.product).first()
                if image:
                    if image.image:
...
def get_ref_number(sender, instance, created, **kwargs):
    print('SIGNAl-->')
    if created:
        distributor_name =instance.distributor.name
        v_ref = "NET" + "00" + str(instance.pk)
        if len(distributor_name) > 1:
            ...
            if len(split_word) > 1:
                ...
                v_ref = str(first_character) + \
                    str(second_character) + "00" + str(instance.pk)
            else:
                v_ref = str(
                    distributor_name[:1]) + str(distributor_name[:2]) + "00" + str(instance.pk)
        else:
            v_ref = str(distributor_name) + "00" + str(instance.pk)

        designation =  v_ref.upper()

        instance.ref_number=designation
        instance.save()
        RetailOrders.objects.filter(pk=instance.pk).update(ref_number=designation)
class Delivery(models.Model):
    distributor = models.ForeignKey("distributor.Distributor", verbose_name="Distributor", on_delete=models.CASCADE)
    order = models.ForeignKey("distributor.RetailOrders", on_delete=models.DO_NOTHING, related_name="delivery_orders")
    delivery_time = models.DateTimeField(verbose_name="Date Delivered",null=True)
    confirmed = models.BooleanField(verbose_name="Was Confirmed", default=False)
    remarks = models.TextField(verbose_name="Reception Remarks", null=True, blank=True)
    transporter = models.CharField(max_length=100, verbose_name="Transporter Name", null=True, blank=True)
    vehicle_no = models.CharField(max_length=100, verbose_name="Vehicle No.", null=True, blank=True)
    transporter_name = models.CharField(max_length=100, verbose_name="Transporter Name", null=True, blank=True)
    transporter_phone = models.CharField(max_length=15, verbose_name="Transporter Phone", null=True, blank=True)
    history = HistoricalRecords()
...
class SalesMan(models.Model):
    ...
    phone = models.CharField(verbose_name="Phone Number", max_length=15, validators=[validate_phone])
    email = models.EmailField(verbose_name="Email", unique=True)
    pic = models.ImageField(verbose_name='Saleman Image', upload_to="salesman/profile/", blank=True, null=True)
    ...
    def retailers_(self):
        ...
        retailers = Retailer.objects.filter(ret_profile_for_dist__in=retailers_)
        return retailers
...
class MobileBanner(models.Model):
    ...
    pic = models.ImageField(null=True, max_length=255, verbose_name="Banner Display Pic", upload_to='mobile/banners/')
    status = models.CharField(max_length=50,
                              choices=(("Banner", "Banner"), ("Product", "Product"), ("Offer", "Offer"),),
                              default="Banner",
                              verbose_name="Banner Status")
    product = models.ForeignKey('distributor.Product', on_delete=models.PROTECT, blank=True, null=True)
    offer = models.ForeignKey('distributor.PriceOffer', on_delete=models.PROTECT, blank=True, null=True)
...
class MNotification(models.Model):
    ...
    display_text = models.CharField(max_length=200, verbose_name="Display Message")
    ...
    pic = models.ImageField(upload_to='notifications/img/', max_length=255, null=True, blank=True)
    status = models.CharField(max_length=50,
                              choices=(
    ...
    product = models.ForeignKey('distributor.Product', on_delete=models.PROTECT, blank=True, null=True)
    offer = models.ForeignKey('distributor.PriceOffer', on_delete=models.PROTECT, blank=True, null=True)
    order = models.ForeignKey("distributor.RetailOrders", blank=True, on_delete=models.PROTECT, null=True)
    ...
    users = models.ManyToManyField("netbotAuth.User", blank=True, related_name="user_notifications")
...
class DistNotifications(models.Model):
    ...
    display_text = models.CharField(verbose_name="Display Text", max_length=100)
...
class ProductPackage(models.Model):
    distributor = models.ForeignKey('distributor.Distributor', on_delete=models.CASCADE)
    product = models.ForeignKey('distributor.Product', on_delete=models.CASCADE, verbose_name="Product")
    ...
    item_no = models.CharField(verbose_name="Items Size/Number", max_length=200, null=True, blank=True)
    price = MoneyField(max_digits=14, decimal_places=2, default_currency='USD')
    image = models.ImageField(upload_to='product/packages/img/', null=True, blank=True)
...
class RetailerType(models.Model):
    ...
    discount = models.PositiveIntegerField(verbose_name="Product Percent Discount")
    retailers = models.ManyToManyField('distributor.Retailer', verbose_name="Retailers", blank=True)
...
class ProductPrice(models.Model):
    distributor = models.ForeignKey('distributor.Distributor', on_delete=models.CASCADE)
    product = models.ForeignKey('distributor.Product', on_delete=models.CASCADE)
    above = models.PositiveIntegerField(verbose_name="Min Quantity")
    below = models.PositiveIntegerField(verbose_name="Max Quantity")
    price = MoneyField(max_digits=14, decimal_places=2, verbose_name="Level Price", default_currency='KES')
...
class PriceList(models.Model):
    ...
    variation = models.ForeignKey(
        "distributor.VariationOptions", verbose_name="Price Variations",on_delete=models.PROTECT,
        null=True, blank=True)

    discount_amount = models.FloatField(verbose_name="Discount Amount if Any", null=True, blank=True, default=0)
    discount_percent = models.FloatField(verbose_name="Discount Percent if Any", null=True, blank=True, default=0)
...
class PriceOffer(models.Model):
    ...
    # x_amt = models.PositiveIntegerField(verbose_name="X Quantity")
    y_amt = models.PositiveIntegerField(verbose_name="Y Quantity", blank=True, null=True)
    date_from = models.DateField(verbose_name="Date From")
    date_to = models.DateField(verbose_name="Date To")
    pic = models.ImageField(verbose_name="Display Pic", upload_to="pics/offers/", blank=True, null=True)
    detail_name = models.CharField(max_length=200, verbose_name="Detail Name", editable=True, null=True, blank=True)
...
class DistRegion(models.Model):
    distributor = models.ForeignKey("distributor.Distributor", related_name="dist_regions", on_delete=models.CASCADE)
...
class DistCity(models.Model):
    distributor = models.ForeignKey("distributor.Distributor", related_name="dist_cities", on_delete=models.CASCADE)
...
class SalesTargetGroup(models.Model):
    distributor = models.ForeignKey("distributor.Distributor", related_name='dist_sales_target_group',
                                    on_delete=models.CASCADE)
    name = models.CharField(verbose_name="Target Group Name", max_length=100, default="group 1")
    salesmen = models.ManyToManyField("distributor.SalesMan", verbose_name="Grouped Saleman")
    ...
    target_sales = MoneyField(max_digits=14, decimal_places=2, verbose_name="Target Sales", default=Money("0", "KES"),
                              default_currency='KES')
    status = models.CharField(max_length=100, verbose_name="Target Status", editable=False, default="")
    end_date = models.DateField(verbose_name="To be Achieved Before", null=True, blank=True, editable=False)
    reward_money = MoneyField(max_digits=14, decimal_places=2, verbose_name="Monetary Reward",
                              default=Money("0", "KES"),
                              default_currency='KES')
    reward_per = models.PositiveIntegerField(verbose_name="Reward in Percentage", null=True, blank=True)
...
class SalesTarget(models.Model):
    distributor = models.ForeignKey("distributor.Distributor", on_delete=models.CASCADE)
    name = models.CharField(verbose_name="Target Name", max_length=100, default="probation")
    salesman = models.ForeignKey("distributor.SalesMan", on_delete=models.CASCADE, related_name="sales_targets")
    period = models.CharField(max_length=20, choices=(("Daily", "Daily"),
                                                      ("Weekly", "Weekly"),
                                                      ("Monthly", "Monthly"),
                                                      ("Yearly", "Yearly")), default="Daily",
...
class RetailerContacts(models.Model):
    retailer = models.ForeignKey("distributor.Retailer", on_delete=models.CASCADE, related_name="retailer_contacts")
    position = models.CharField(verbose_name="Employee Position", max_length=50, null=True, blank=True)
    email = models.EmailField(verbose_name="Email", max_length=50)
    phone = models.CharField(verbose_name="Phone No", max_length=13, validators=[validate_phone, ])
...
class CustomColors(models.Model):
    value = models.TextField(verbose_name='Color values', blank=True, null=True)
    label = models.CharField(verbose_name='Color Label', max_length=100, null=True, blank=True)
    color = models.CharField(verbose_name='Color Hex', max_length=100, null=True, blank=True)
...
class PartialOrders(models.Model):
    ...
    qty = models.PositiveIntegerField(verbose_name="Quantity")
    order_price = MoneyField(max_digits=14, decimal_places=2, verbose_name="Order Price", default_currency='KES')
    dist_order = models.ForeignKey("distributor.DistOrder", verbose_name="Product", on_delete=models.PROTECT)
...
class PartialDelivery(models.Model):
    ...
    current_orders = models.ManyToManyField("distributor.PartialOrders", verbose_name="Current Orders")
    confirmed = models.BooleanField(verbose_name="Was Confirmed", default=False)
    remarks = models.TextField(verbose_name="Reception Remarks", null=True, blank=True)
    transporter = models.CharField(max_length=100, verbose_name="Transporter Name", null=True, blank=True)
    vehicle_no = models.CharField(max_length=100, verbose_name="Vehicle No.", null=True, blank=True)
    transporter_name = models.CharField(max_length=100, verbose_name="Transporter Name", null=True, blank=True)
    transporter_phone = models.CharField(max_length=15, verbose_name="Transporter Phone", null=True, blank=True)
    when_dispatched = models.DateTimeField(auto_now_add=True, null=True, editable=False)
...
class RetailerProfile(models.Model):
    ...
    retailer = models.ForeignKey("distributor.Retailer", verbose_name="Retailer", related_name="ret_profile_for_dist",
                                 null=True, blank=True, on_delete=models.PROTECT)
    contact_name = models.CharField(verbose_name="CRetailer lient Name", max_length=100)
    contact_phone = models.CharField(max_length=15, verbose_name="Retailer Profile Phone", validators=[validate_phone])
    contact_email = models.EmailField(max_length=50, verbose_name="Retailer Email")
    contact_pic = models.ImageField(verbose_name='Retailer Contact Image', upload_to="retailer/dist/", blank=True,
                                    null=True)
    contact_person = models.CharField(verbose_name="Retailer contact person", max_length=100, null=True, blank=True)
    contact_details = models.CharField(verbose_name="Contact Person details", max_length=100, null=True, blank=True)
    ...
    contact_person_position = models.CharField(verbose_name="Contact person position", max_length=100, null=True,
                                               blank=True)
    contact_city = models.TextField(verbose_name="contact city", null=True, blank=True)
    ...
    latitude = models.CharField(verbose_name='Retailer Latitude', max_length=100, null=True, blank=True)
    longitude = models.CharField(verbose_name='Retailer Longitude', max_length=100, null=True, blank=True)
    county = models.CharField(verbose_name='Retailer curent count', max_length=100, null=True, blank=True)
    world_country = models.CharField(verbose_name='Retailer current country', max_length=100, null=True, blank=True)
...
class DistEmailSettings(models.Model):
    ...
    email_port = models.PositiveIntegerField(verbose_name="Email Port", default="465")
    email_user = models.EmailField(verbose_name="Email Username")
    email_password = models.CharField(verbose_name="Email Password", max_length=100)
    ...
    def save(self, *args, **kwargs):
        if self.can_send_email:
            ...
            raise ValidationError("Your Email Configuration Might be Wrong, We could Not Send A test Email")
...
class DistributorOptions(models.Model):
    ...
    use_price_list = models.BooleanField(verbose_name="Use Price List", default=True)
    use_sales_target = models.BooleanField(verbose_name="Use Sales Target", default=True)
    use_offers = models.BooleanField(verbose_name="Use Offers", default=True)
    about_company = models.TextField(verbose_name="About Company", null=True, blank=True)
    terms_company = models.TextField(verbose_name="Terms and conditions", null=True, blank=True)
    privacy_company = models.TextField(verbose_name="Privacy and Policy", null=True, blank=True)
...
class Brand(models.Model):
    distributor = models.ForeignKey("distributor.Distributor", verbose_name="Distributor", on_delete=models.PROTECT)
    ...
    pic = models.ImageField(max_length=500, upload_to="pics/brand_pics/", verbose_name="Brand Pic", null=True,
                            blank=True)
    description = models.CharField(max_length=200, verbose_name="Brief Description")
...
class Variation(models.Model):
    ...
    product = models.ForeignKey(
        "distributor.Product", related_name="product_variations", on_delete=models.CASCADE, null=True, blank=True)
...
class VariationOptions(models.Model):
    ...
    distributor = models.ForeignKey(
        "distributor.Distributor", related_name="dist_variation_options", on_delete=models.CASCADE, unique=False)
    variation = models.ForeignKey("distributor.Variation", related_name="attributes", verbose_name="Variation",
                                  on_delete=models.PROTECT, blank=True, null=True, unique=False)
    value = models.JSONField(verbose_name="Variation Options", blank=True, null=True, unique=False)
    ...
    images = models.ManyToManyField(
        "distributor.VariationOptionImages", related_name="variation_images", blank=True)
    videos = models.ManyToManyField(
        "distributor.VariationOptionVideos", related_name="variation_videos", blank=True)
...
class VariationImage(models.Model):
        ...
        variation = models.ForeignKey("distributor.Variation", on_delete=models.CASCADE, null=True, blank=True)
        variant = models.ForeignKey("distributor.VariationOptions", on_delete=models.CASCADE, null=True, blank=True)
        image = models.ImageField(upload_to="variant/images/", null=True, blank=True)
...
class VariationVideo(models.Model):

        variation = models.ForeignKey("distributor.Variation", on_delete=models.CASCADE, null=True, blank=True)
        variant = models.ForeignKey("distributor.VariationOptions", on_delete=models.CASCADE, null=True, blank=True)
        video = models.ImageField(upload_to="variant/videos/", null=True, blank=True) 

        class Meta:
            db_table = "variant_video"
            verbose_name = "Variant Video" 
            verbose_name_plural = "Variant Videos" 


class VariationOptionImages(models.Model):

    variation_option =  models.ForeignKey("distributor.VariationOptions", on_delete=models.CASCADE, related_name="variation_options_images", null=True, blank=True)
    image = models.ImageField(upload_to="variants/images/", null=True, blank=True) 
    product = models.ForeignKey("distributor.Product", on_delete=models.CASCADE, related_name="variant_images", null=True, blank=True)

    class Meta:
        db_table = "vo_images"
        verbose_name = "Variation Option Image"
        verbose_name_plural = "Variation Option Images"   

class VariationOptionVideos(models.Model):

    variation_option = models.ForeignKey("distributor.VariationOptions", on_delete=models.CASCADE, related_name="variation_options_videos", null=True, blank=True)
    video = models.ImageField(upload_to="variants/videos/", null=True, blank=True) 
    product = models.ForeignKey("distributor.Product", on_delete=models.CASCADE, related_name="variant_videos", null=True, blank=True)

    class Meta:
        db_table = "vo_videos"
        verbose_name = "Variation Option Videos"
        verbose_name_plural = "Variation Option Videos"  