...
from distributor.models import validate_phone, validate_pin, DistributorOptions
...
class Distributor(models.Model):
    ...
    kra = models.CharField(max_length=11, unique=True, verbose_name="Distributor Tax Pin", validators=[validate_pin, ]
                           )
    ...
    contact_phone = models.CharField(verbose_name="Contact Person Phone",
                                     max_length=15, validators=[validate_phone])
    website = models.CharField(verbose_name="Website", blank=True, max_length=200)
    ...
    category = models.CharField(verbose_name="Specialized Goods", max_length=100)
    ...
    when_created = models.DateTimeField(verbose_name="When Created", auto_now_add=True, null=True, editable=False)
    ...
    def dist_details(self):
        about = dict()
        dist_options = DistributorOptions.objects.filter(distributor_id=self.id).first()
        if dist_options:
...
class NetBotNotification(models.Model):
    ...
    seen_by = models.ManyToManyField("netbotAuth.User", related_name="seen_notifications")
    not_seen_by = models.ManyToManyField("netbotAuth.User", related_name="not_seen_notifications")
...
class WorldCountry(models.Model):
    ...
    cities = models.ManyToManyField("distributor.WorldCity", verbose_name="Country Cities", blank=True, editable=False)
...
class ApkStorage(models.Model):
    apk = models.FileField(upload_to="android/apk", verbose_name="Android Apk", max_length=500, null=True)
