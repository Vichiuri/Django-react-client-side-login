...
def create_distributor_notification(sender, instance, created, **kwargs):
    not_seen_by = User.objects.filter(is_superuser=True).all()

    if created:
        distName = instance.name
        action_by = instance.created_by
        text = str(action_by) + ' added distributor ' + distName + ' state.'
        details = 'Distributor ' + distName + ' was added by ' + str(
            action_by) + '. \n\n Distributor Email: ' + instance.email + '.'
        ...
        previous = Distributor.objects.get(id=instance.id)

        if previous != instance:
            distName = instance.name
            action_by = instance.created_by
            text = str(action_by) + ' updated distributor ' + distName + ' '
...
def create_subscription_notification(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        if previous == instance:
            subName = instance.name
            action_by = instance.created_by
            text = str(action_by) + ' updated "' + subName + '" subscription'
...
@receiver(post_save, sender=NetBotUser)
def create_user_notification(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        details = str(
            ...
            instance.phone) + ' and permission group ' + str(instance.permission_group)

        p = NetBotNotification(display_text=text, details=details, action_by=instance, when_created=timezone.now())
        p.save()
        for ns in not_seen_by:
            p.not_seen_by.add(ns)
            p.save()
        ...
        previous = NetBotUser.objects.get(id=instance.id)
        if previous == instance:
            netbotUserFName = instance.first_name
            netbotUserLName = instance.last_name
            action_by = instance.user
